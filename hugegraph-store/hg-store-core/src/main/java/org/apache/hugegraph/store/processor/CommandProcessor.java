...
public abstract class CommandProcessor {
    ...
    /**
     * Queue the commands for partitioning according to the partitioning scheme.
     */
    private static final Map<Integer, BlockingDeque<Runnable>> TASKS = new ConcurrentHashMap<>();

    /**
     * Execution status of partition tasks (whether they are being executed)
     */
    ...
    /**
     * Is there any task data to be processed by the modified process?
    ...
    /**
     * Whether to execute via a thread pool (to be blocked within the partition)
    ...
    /**
     * If it is a leader, directly add and send the raft task; otherwise, redirect to the leader.
     *
     * @param partId partition id
     ...
     */
    protected void sendRaftTask(String graph, Integer partId, byte raftOp, Object data, RaftClosure closure) {
        ...
        if (partitionEngine != null) {
            if (partitionEngine.isLeader()) {
               partitionEngine.addRaftTask(RaftOperation.create(raftOp, data), closure);
                ...
                var response = storeEngine.getCmdClient().redirectRaftTask(request);
                closure.run(response.getStatus().isOK() ? Status.OK() :
                        new Status(response.getStatus().getCode(), response.getStatus().getMsg()));
            }
        }
    }
    ...
    /**
     * Is there an instruction being executed?
    ...
    /**
     * Are there any more commands in the queue?
